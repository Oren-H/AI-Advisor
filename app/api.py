...
class ChatResponse(BaseModel):
    ...
    intent: str = Field(..., description="Intent (placeholder)")
    course_results: Optional[List[Dict[str, Any]]] = Field(None, description="Placeholder course results")
    filters: Optional[Dict[str, Any]] = Field(None, description="Placeholder filters")
...
async def health_check():
    return HealthResponse(
        ...
        graph_ready=True,  # no external graph in this minimal server
    )
...
def _invoke_agent(conv_id: str, prompt: str, user_profile: Dict[str, Any]) -> str:
    ...
    allowed_keys = {"department_of_major", "major", "completed_courses", "career_goals", "semester", "preferences"}
...
async def chat_stream(request: ChatRequest):
    async def generate_stream() -> AsyncGenerator[str, None]:
        try:
            ...
            # Naive tokenization for streaming UX
            for token in full_reply.split(" "):
                yield f"data: {json.dumps({'type': 'token', 'content': token + ' '})}\n\n"
                await asyncio.sleep(0.005)  # tiny delay for UI experience

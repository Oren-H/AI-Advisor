...
class IntentRoute(BaseModel):
    ...
    intent: Optional[Literal["major_search", "course_search", "general"]] = Field(default=["general"], 
        description="Figuring out the intents of the user query") 
    reasoning: Optional[str] = Field(default="", description="Brief explanation of why these intents were chosen") 
...
# node 
def synthesize(state: CourseAdvisorState):
    """Synthesize the agent's response."""
    # take the results and synthesize a response

    prompt = f"""
    Synthesize the agent's response based on the user query and the information provided. 
    Query from the user: {state.user_query}
    Information provided: {state}
    """
...
workflow.add_edge("course_search", "synthesize")
workflow.add_edge("major_search", "synthesize")
workflow.add_edge("general_response", "synthesize")

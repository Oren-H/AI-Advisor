...
def scrape_department(url, department_name=None):
    ...
    if overview_container:
        ...
        dept_name = clean_text(first_h2.get_text()) if first_h2 else (department_name or "Unknown Department")
    ...
    for blk in blocks:
        ...
        for node in blk["nodes"]:
            if node.name == "table" and "sc_courselist" in node.get("class", []):
                majors[major_id]["course_lists"].append(parse_table(node))
            elif node.name == "table" and "sc_prerequisite" in node.get("class", []):
                majors[major_id]["cognates"].append(parse_table(node))
